...
def load_data():
    with open(
        "./profile_service_response.json", encoding="utf-8"
    ) as profile_file:
        profile_json = json.load(profile_file)

    with open(
        "./posts_management_service_response.json", encoding="utf-8"
    ) as posts_file:
        posts_json = json.load(posts_file)

    with open(
        "./moderation_service_response.json", encoding="utf-8"
    ) as moderation_file:
    ...
    with open("plantuml.txt", "w", encoding="utf-8") as pf:
        ...
        for moderated_post in moderation_data:
            pf.write("post_" + str(moderated_post["post_id"]) +
                     " : is_reported = " +
                     str(moderated_post["is_reported"]) + "\n")
            pf.write("post_" + str(moderated_post["post_id"]) +
                     " : is_manual = " +
                     str(moderated_post["is_manual"]) + "\n")
            pf.write("post_" + str(moderated_post["post_id"]) +
                     " : reason = " + moderated_post["reason"] + "\n")

            if "reported_by" in moderated_post:
                pf.write(
                    "post_" + str(moderated_post["post_id"]) +
                    " : reported_by = " + moderated_post["reported_by"] + "\n")
        pf.write("@enduml \n")
